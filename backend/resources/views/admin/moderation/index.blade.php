@extends('layouts.admin')

@section('content')
<div class="flex items-center justify-between mb-4">
    <h1 class="text-xl font-semibold text-black">Модерация</h1>
    <div class="text-xs text-neutral-500">
        одобрено: <span class="text-black font-medium">{{ $counts['approved'] }}</span>
    </div>
</div>

<div class="flex items-center gap-1 mb-4">
    @php
        $tabs = [
            'queue' => ['Очередь', $counts['pending'] + $counts['rejected']],
            'rejected' => ['Нарушения', $counts['rejected']],
            'pending' => ['Не проверено', $counts['pending']],
        ];
    @endphp
    @foreach ($tabs as $key => [$label, $count])
        <a href="{{ route('admin.moderation.index', ['filter' => $key]) }}" class="flex items-center gap-1.5 px-3 py-1.5 rounded-lg text-sm transition {{ $filter === $key ? 'bg-black text-white' : 'text-neutral-600 hover:bg-neutral-100' }}">
            {{ $label }}
            <span class="text-xs {{ $filter === $key ? 'text-neutral-300' : 'text-neutral-400' }}">{{ $count }}</span>
        </a>
    @endforeach
</div>

<div class="space-y-3">
    @forelse ($posts as $post)
        <div class="border border-neutral-200 rounded-lg p-4 bg-white">
            <div class="flex items-start justify-between gap-4 mb-2">
                <div class="flex items-center gap-2 flex-wrap min-w-0">
                    <x-username :user="$post->user" class="text-sm" />
                    <span class="text-xs text-neutral-400">в</span>
                    <a href="{{ route('topics.show', $post->topic) }}#post-{{ $post->id }}" class="text-xs text-neutral-500 hover:text-black hover:underline truncate">{{ $post->topic->title }}</a>
                    @if ($post->moderation_status === 'rejected')
                        <span class="text-[10px] uppercase tracking-wide bg-red-50 text-red-600 px-1.5 py-0.5 rounded">нарушение</span>
                    @else
                        <span class="text-[10px] uppercase tracking-wide bg-neutral-100 text-neutral-500 px-1.5 py-0.5 rounded">не проверено</span>
                    @endif
                </div>

                <div class="text-right shrink-0">
                    @php $score = (float) $post->confidence_score; @endphp
                    <div class="text-sm font-semibold {{ $score >= 0.8 ? 'text-red-600' : ($score >= 0.5 ? 'text-amber-600' : 'text-neutral-500') }}">
                        {{ number_format($score * 100, 1) }}%
                    </div>
                    <div class="text-[10px] text-neutral-400 uppercase tracking-wide">токсичность</div>
                </div>
            </div>

            <div class="h-1 bg-neutral-100 rounded-full overflow-hidden mb-3">
                <div class="h-full rounded-full {{ $score >= 0.8 ? 'bg-red-500' : ($score >= 0.5 ? 'bg-amber-500' : 'bg-neutral-400') }}" style="width: {{ min(100, $score * 100) }}%"></div>
            </div>

            <div class="text-sm text-neutral-800 whitespace-pre-line leading-relaxed bg-neutral-50 rounded-lg px-3 py-2">{{ $post->body }}</div>

            <div class="flex items-center justify-between mt-3">
                <div class="text-xs text-neutral-400">{{ $post->created_at->timezone('Asia/Irkutsk')->format('d.m.Y H:i') }}</div>

                <div class="flex items-center gap-2">
                    @if ($post->moderation_status !== 'approved')
                        <form method="POST" action="{{ route('admin.moderation.approve', $post) }}">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="flex items-center gap-1.5 text-xs text-green-700 border border-green-200 bg-green-50 rounded-lg px-3 py-1.5 hover:bg-green-100 transition">
                                <i class="ti ti-check text-sm"></i> Одобрить
                            </button>
                        </form>
                    @endif

                    @if ($post->moderation_status !== 'rejected')
                        <form method="POST" action="{{ route('admin.moderation.reject', $post) }}">
                            @csrf
                            @method('PATCH')
                            <button type="submit" class="flex items-center gap-1.5 text-xs text-neutral-600 border border-neutral-200 rounded-lg px-3 py-1.5 hover:bg-neutral-50 transition">
                                <i class="ti ti-eye-off text-sm"></i> Скрыть
                            </button>
                        </form>
                    @endif

                    <button type="button" class="flex items-center gap-1.5 text-xs text-red-600 border border-red-200 rounded-lg px-3 py-1.5 hover:bg-red-50 transition"
                        onclick="openDeleteModal('{{ route('posts.destroy', $post) }}', 'Удалить сообщение?', 'Сообщение будет удалено безвозвратно.')">
                        <i class="ti ti-trash text-sm"></i> Удалить
                    </button>
                </div>
            </div>
        </div>
    @empty
        <div class="border border-neutral-200 rounded-lg py-12 text-center bg-white">
            <i class="ti ti-checkbox text-3xl text-neutral-300"></i>
            <div class="text-sm text-neutral-400 mt-2">Очередь пуста — всё проверено</div>
        </div>
    @endforelse
</div>

<div class="mt-4">{{ $posts->links() }}</div>
@endsection