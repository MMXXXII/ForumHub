    ...
    <div class="flex min-h-screen">
        <aside class="w-60 shrink-0 bg-white border-r border-neutral-200 flex flex-col">
            ...
            <nav class="flex-1 px-2 py-4 space-y-1 text-sm">
                ...
                <a href="{{ route('admin.posts.index') }}" class="flex items-center justify-between px-2 py-1.5 rounded {{ request()->routeIs('admin.posts.*') ? 'bg-black text-white' : 'text-neutral-700 hover:bg-neutral-100' }}">
                    <span>Сообщения</span>
                    @if ($queueCount > 0)
                        <span class="text-xs {{ request()->routeIs('admin.posts.*') ? 'text-neutral-300' : 'bg-red-50 text-red-600 px-1.5 rounded' }}">{{ $queueCount }}</span>
                    @endif
                </a>
